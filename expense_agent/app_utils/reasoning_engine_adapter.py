...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    async def query(request: Request) -> responses.JSONResponse:
        ...
        if class_method == "query":
            # ADK workflows only support streaming queries. We adapt the synchronous
            # query by running the async_stream_query to completion.
            ...
            async for event in method(**kwargs):
                if isinstance(event, dict):
                    ...
                    output = getattr(event, "output", None) or output
        else:
